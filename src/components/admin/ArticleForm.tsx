...
export default function ArticleForm({ initialData, categories, authors }: ArticleFormProps) {
  ...
  const showAlert = (message: string) => {
    setDialogConfig({ isOpen: true, title: "Анхааруулга", message, isAlert: true });
  };

  const handlePdfUpload = async (e: React.ChangeEvent<HTMLInputElement>, field: "pdfUrl" | "pdfUrlEn" | "pdfUrlZh") => {
      ...
      <form onSubmit={handleSubmit} className="space-y-8 bg-white p-6 md:p-8 rounded-xl shadow-sm border border-slate-100">
        ...
        <div className="flex justify-between items-center bg-blue-50 p-4 rounded-lg border border-blue-100">
          <div>
            ...
            <p className="text-sm text-slate-600">Монголоор бичсэн мэдээллээ автоматаар Англи, Хятад хэл рүү орчуулах</p>
          </div>
          <button 
            type="button" 
        ...
        <div className="space-y-6 pt-4 border-t">
          <h3 className="text-lg font-bold text-[#002b5c] border-b pb-2">Англи хэлээр (EN)</h3>
          <div>
            <label className="block text-sm font-bold text-slate-700 mb-2">Гарчиг (EN)</label>
            <input type="text" value={formData.titleEn} onChange={e => setFormData({...formData, titleEn: e.target.value})} className="w-full px-4 py-2 border rounded-lg" />
          </div>
          <div>
            <label className="block text-sm font-bold text-slate-700 mb-2">Агуулга (EN)</label>
            <div className="h-64 mb-12">
              <ReactQuill theme="snow" value={formData.contentEn} onChange={val => setFormData({...formData, contentEn: val})} className="h-full" />
            </div>
          </div>
        </div>

        <div className="space-y-6 pt-4 border-t">
          <h3 className="text-lg font-bold text-[#002b5c] border-b pb-2">Хятад хэлээр (ZH)</h3>
          <div>
            <label className="block text-sm font-bold text-slate-700 mb-2">Гарчиг (ZH)</label>
            <input type="text" value={formData.titleZh} onChange={e => setFormData({...formData, titleZh: e.target.value})} className="w-full px-4 py-2 border rounded-lg" />
          </div>
          <div>
            <label className="block text-sm font-bold text-slate-700 mb-2">Агуулга (ZH)</label>
            <div className="h-64 mb-12">
              <ReactQuill theme="snow" value={formData.contentZh} onChange={val => setFormData({...formData, contentZh: val})} className="h-full" />
            </div>
          </div>
        </div>

        <div className="space-y-6 pt-4 border-t">
          <h3 className="text-lg font-bold text-[#002b5c] border-b pb-2">PDF Файлууд (Хэл тус бүрээр)</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="border p-4 rounded-lg bg-slate-50">
              ...
              {formData.pdfUrl ? (
                <div className="flex items-center gap-2">
                  <a href={formData.pdfUrl} target="_blank" rel="noreferrer" className="text-xs text-blue-600 hover:underline truncate w-32">{formData.pdfUrl.split('/').pop()}</a>
                  <button type="button" onClick={() => setFormData({...formData, pdfUrl: ""})} className="text-red-500"><X className="w-4 h-4" /></button>
                </div>
              ) : (
                <input type="file" accept="application/pdf" className="text-xs" onChange={(e) => handlePdfUpload(e, "pdfUrl")} />
              )}
            </div>
            ...
            <div className="border p-4 rounded-lg bg-slate-50">
              ...
              {formData.pdfUrlEn ? (
                <div className="flex items-center gap-2">
                  <a href={formData.pdfUrlEn} target="_blank" rel="noreferrer" className="text-xs text-blue-600 hover:underline truncate w-32">{formData.pdfUrlEn.split('/').pop()}</a>
                  <button type="button" onClick={() => setFormData({...formData, pdfUrlEn: ""})} className="text-red-500"><X className="w-4 h-4" /></button>
                </div>
              ) : (
                <input type="file" accept="application/pdf" className="text-xs" onChange={(e) => handlePdfUpload(e, "pdfUrlEn")} />
              )}
            </div>
            ...
            <div className="border p-4 rounded-lg bg-slate-50">
              ...
              {formData.pdfUrlZh ? (
                <div className="flex items-center gap-2">
                  <a href={formData.pdfUrlZh} target="_blank" rel="noreferrer" className="text-xs text-blue-600 hover:underline truncate w-32">{formData.pdfUrlZh.split('/').pop()}</a>
                  <button type="button" onClick={() => setFormData({...formData, pdfUrlZh: ""})} className="text-red-500"><X className="w-4 h-4" /></button>
                </div>
              ) : (
                <input type="file" accept="application/pdf" className="text-xs" onChange={(e) => handlePdfUpload(e, "pdfUrlZh")} />
              )}
            </div>
          </div>
        </div>
