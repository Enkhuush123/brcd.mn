...
export default function MessagesClient({ initialMessages }: { initialMessages: any[] }) {
  ...
  const handleMarkAsRead = async (id: string) => {
    try {
      ...
      if (res.ok) {
        ...
        setDialogConfig({ isOpen: true, title: "Алдаа", message: "Үйлдэл амжилтгүй.", isAlert: true });
      }
    } catch (err) {
  ...
  const requestDelete = (id: string) => {
    setDialogConfig({
      isOpen: true,
      title: "Анхааруулга",
      message: "Энэ зурвасыг устгахдаа итгэлтэй байна уу?",
      isAlert: false,
      idToDelete: id
    });
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setDialogConfig({ isOpen: true, title: "Алдаа", message: "Устгаж чадсангүй.", isAlert: true });
      }
    } catch (err) {
      setDialogConfig({ isOpen: true, title: "Алдаа", message: "Алдаа гарлаа", isAlert: true });
    } finally {
            ...
            {messages.map(msg => (
              <tr key={msg.id} className={`transition-colors ${msg.isRead ? 'bg-white' : 'bg-blue-50/50'}`}>
                ...
                <td className="px-6 py-4 text-right space-x-2 w-32">
                  ...
                  <button onClick={() => requestDelete(msg.id)} title="Устгах" className="p-2 text-red-600 hover:bg-red-50 rounded-lg">
